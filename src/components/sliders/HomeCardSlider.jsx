...
const products = [
  {
    ...
    discountCircle:
      "top-24 right-20 lg:top-36 lg:right-32 2xl:top-40 2xl:right-48",
  },
  {
    name: "Green jacket",
    ...
    rotationSplash: "-rotate-first",
    discountCircle:
      "top-20 right-14 lg:top-28 lg:right-28 2xl:top-32 2xl:right-44",
  },
  {
    name: "Green jacket",
    ...
    discountCircle:
      "top-20 right-12 lg:top-32 lg:right-24 2xl:top-40 2xl:right-36",
  },
];
...
export const HomeCardSlider = () => {
  ...
  const next = () => {
    setIndexArray((indexArray + 1) % products.length);
    useChangeClassName(setIsDisabled, "fadeInNext", boxImage, 1000);
  };

  const previous = () => {
    setIndexArray((indexArray > 0 ? indexArray : products.length) - 1);
    useChangeClassName(setIsDisabled, "fadeInNext", boxImage, 1000);
  };
  return (
    <div className="relative w-80 h-96 m-auto pt-10 lg:mb-0 lg:mr-0 lg:w-[35rem] lg:h-full 2xl:w-[45rem]">
      <div className="w-full h-full" ref={boxImage}>
        <span
          className={`${products[indexArray].rotationSplash} absolute translate-y-1/2 -translate-x-1/2 rounded-full bg-darkEsmerald w-72 h-48 lg:w-full lg:h-80 2xl:translate-y-2/3 2xl:w-5/6`}
        />
        <img
